...
void Tree::deleteNode(int number) {
  ...
  int nodeThere = 0;
  //CASE IF NOT YOU'RE DELETING IS BLACK
  if (replaceNode == NULL) {
  ...
  //run cases for the Node that's getting deleted- which is replaceNode

  //if replaceNode is red, just replace with child
  if (replaceNode != NULL) {
  ...
  if (replaceNode->color == 0) {
    ...
    else if (replaceNode->right != NULL) {
      replaceNode->parent->left = replaceNode->right;
      replaceNode->right->parent = replaceNode->parent;
    }
    else {
  ...
  //if replaceNode is black and child is red, replace w/ child and make child black
  if (replaceNode != NULL) {
  if (replaceNode->color == 1) {
    if (replaceNode->left != NULL) {
      if (replaceNode->left->color == 0) {
	if (replaceNode->parent->right == replaceNode) {
	replaceNode->parent->right = replaceNode->left;
	}
	else {
	...
	replaceNode->left->parent = replaceNode->parent;
	replaceNode->left->color = 1;
	return;
      }
    }
    if (replaceNode->right != NULL) {
      if (replaceNode->right->color == 0) {
	...
	replaceNode->right->parent = replaceNode->parent;
	replaceNode->right->color = 1;
	return;
      }
    }
  }
  }
  //if replaceNode is black with no children, run cases
  if (replaceNode == NULL) {
  ...
  if (replaceNode->color == 1 && replaceNode->left == NULL && replaceNode->right == NULL) {
...
void Tree::cases(Node* N, Node* P) {
  ...
  //get sibling and sibling's children
  Node* S = NULL;
  ...
  //display(head,0);
  //case 3
  //if N, P, S, SL, and SR are all black, change S to R
  if (S != NULL) {
    if (S->color == 1 && P->color == 1) {
      if (N == NULL) {
	//check SL and SR if exist
	if (SL != NULL) {
	  if (SL->color == 1) {
	    if (SR != NULL) {
	      if (SR->color == 1) {
  ...
  //display(head,0);
  //case 4
  //if S, SL, and SR are black and P is red, make P black and S red
  if (P->color == 0 && S != NULL) {
    if (S->color == 1) {
      if (SL != NULL) {
	if (SL->color == 1) {
  ...
  //case 5
  //if S is black, SL is red, SR is black, and N is the left child
  if (P->left == N && S != NULL && SL != NULL) {
    if (S->color == 1 && SL->color == 0) {
      if (SR != NULL) {
	if (SR->color == 1) {
	  //ROTATE
	  SL->parent = P;
  ...
  //case 5 if N is right child
  else if (P->right == N && S != NULL && SR != NULL) {
    if (S->color == 1 && SR->color == 0) {
      if (SL != NULL) {
	if (SL->color == 1) {
	  //ROTATE
	  S->right = NULL;
  ...
  //case 6
  //N is left child, S is black, and SR is red
  if (P->left == N && S != NULL && SR != NULL) {
    if (S->color == 1 && SR->color == 0) {
      if (G == NULL) {
	head = S;
      }
  ...
  //N is right child, S is black, and SL is red
  else if (P->right == N && S != NULL && SL != NULL) {
    if (S->color == 1 && SL->color == 0) {
      if (G == NULL) {
	head = S;
      }
